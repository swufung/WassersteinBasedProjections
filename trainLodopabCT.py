...
n_train         = 20000  # number of training samples (10000 max)
n_val           = 2000  # number of validation (1000 max)
batch_size = 32        # batch size for training dataset
batch_size_val  = 100   # batch size for validation set
...
# distance to manifold term (regularizer J(u) = 0 on true manifold)
...
do_augment = False
n_train_augmented = n_train # number of samples when data is augmented (for averaging)

mu = 0.5 # relaxation parameter
gp_lam = 50  # gradient penalty strength
...
if not os.path.exists(os.path.dirname('CTLodopab/experimentsLodopab/')):
    print("created CTLodopab/experimentsLodopab/")
    print("os.getcwd()", os.getcwd())
    os.makedirs(os.path.dirname('CTLodopab/experimentsLodopab/'))
else:
    print("experimentsLodopab already exists")

save_path = './CTLodopab/experimentsLodopab/' + 'lodopabCT_CNN_' + utils.get_time_string()  + \
...
if not os.path.exists(os.path.dirname(save_path)):
    ...
    print("CTLodopab/experimentsLodopab/" + " already exists")
    plt.savefig(save_path + '/flow_00.png')
...
wass_dist_val = 0.0
dist_man_val = 0.0
dist_man_batch_sum_val = torch.FloatTensor([0.0])
eta_old = torch.FloatTensor([0.0]);
eta = torch.FloatTensor([eta_tol])  # initialize etas s.t. their diff >= eta_tol
J_real_mean = 0.0 # mean of J on real manifold. used for step size computation later
...
for epoch in range(1, max_epochs + 1):  # loop over the dataset multiple times
    ...
    if gen_iter < total_steps:
        ...
        for i, data in enumerate(data_loader, 0):
            ...
            # gradient penalty
            if do_grad_pen:
                # do gradient penalty only on batch size (no augmentation)
            ...
            if do_manifold_distance:
                ...
                dist_man_batch_sum += (torch.abs(disc_real)).detach().cpu().sum().item()
        ...
        if (gen_iter == 1 and (epoch - epoch_update) % (2 * gen_freq) == 0) \
            ...
            his_dist_man.append(dist_man) # history of distance to manifold
            ...
            for i, data in enumerate(data_loader, 0):
                ...
                # picking step size type: mean beta_k, combined, or individual
                if  step_type == 'Mean':
                    lam = mu * (his_wass[gen_iter - 1])
                elif step_type == 'Combined':
                    lam = mu * (his_wass[gen_iter - 1] + torch.max(Jout.detach() - dist_man, torch.zeros(Jout.size(), device=device)))
                elif step_type == 'Individual':
                    lam = mu * (torch.max(Jout.detach() - dist_man, torch.zeros(Jout.size(), device=device)))
            ...
            for i, data in enumerate(data_loader_val, 0):
                ...
                # step size, mean, combined, or individual
                if  step_type == 'Mean':
                    lam = mu * (his_wass[gen_iter - 1])
                elif step_type == 'Combined':
            ...
            dist_man_val  = 0.999 * dist_man_val + 0.001 * dist_man_batch_sum_val/n_val  # exponential averaging
            ...
            # ----------------------------------------------------------------------------------------------------------
            gen_iter += 1
            J_real_mean = 0.0 # reset mean of J on true manifold
