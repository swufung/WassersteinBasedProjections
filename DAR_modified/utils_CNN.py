...
def generate_data_CNN(n_samples, fwd_operator, fbp_operator, space, noise_level, scale, validation=False):  # output (nFeat1*nFeat2 x nSamples)
    ...
    for i in range(n_generate):
        ...
        # noisy_data = data
        # noisy_data = data + np.random.normal(fwd_operator.range.shape[0], fwd_operator.range.shape[1]) * 0.01
        # noisy_data = data + odl.phantom.white_noise(fwd_operator.range) * 0.01

        fbp_arr[i, ..., 0] = fbp_operator(noisy_data)
        x_true_arr[i, ..., 0] = phantom
        y_arr[i, ..., 0] = noisy_data

    print("1% mean data noise")
    ...
    # x_true_arr = x_true_arr.squeeze(3)
    # y_arr = y_arr.squeeze(3)
    # fbp_arr = fbp_arr.squeeze(3)
    #
    # x_true_arr = x_true_arr.reshape(n_generate, -1)
    # y_arr = y_arr.reshape(n_generate, -1)
    # fbp_arr = fbp_arr.reshape(n_generate, -1)

    # output (nFeat1*nFeat2 x nSamples)
    # x_true_arr = torch.FloatTensor(x_true_arr).permute(1, 0)
    # y_arr = torch.FloatTensor(y_arr).permute(1, 0)
...
def plot_and_save_ellipse_images(u_true_train, u_gen0_train, u_gen_train, u_true_val, u_gen0_val, u_gen_val,
    ...
    plt.subplot(2, 3, 1)
    im = plt.imshow(u_true_train.reshape(n_mesh, n_mesh).detach(), vmin=vmin, vmax=vmax);
    plt.title("u_true train")  # plt.colorbar();
    plt.subplot(2, 3, 2)
    im = plt.imshow(u_gen0_train.reshape(n_mesh, n_mesh).detach(), vmin=vmin, vmax=vmax);
    plt.title("u_gen train 0")  # plt.colorbar();
    plt.subplot(2, 3, 3)
    im = plt.imshow(u_gen_train.reshape(n_mesh, n_mesh).detach(), vmin=vmin, vmax=vmax);
...
def plot_and_save_ellipse_histories(his_wass, his_ssim, his_psnr, his_MSE_val, his_eta, gen_iter,save_path):
...
class CustomTensorDataset(torch.utils.data.Dataset):
    ...
    """
    def __init__(self, tensors_true, tensors_ugen, transforms=None):
        assert all(tensors_true[0].size(0) == tensor.size(0) for tensor in tensors_true)
        self.tensors_true = tensors_true

        assert all(tensors_ugen[0].size(0) == tensor.size(0) for tensor in tensors_ugen)
        self.tensors_ugen = tensors_ugen

        self.transforms = transforms

    def __getitem__(self, index):
        x = self.tensors_true[index]
        y = self.tensors_ugen[index]
    ...
    def __len__(self):
        return self.tensors_true.size(0)
